 ...
 *
 * Click the overlay to enter interactive mode (dropdowns, buttons work).
 * Click outside to exit interactive mode.
 *
 * Supports an immersive fullscreen mode triggered via the `expand` action
 * dispatched from WidgetChrome.
 */
...
export default forwardRef(function ComponentWidget({
  widgetId,
  component: Component,
  jsxModule,
  exportName,
  ...
  const expandKey = widgetId || (exportName ? `jsx-${exportName}` : null)
  const [expandMode, setExpandMode] = useExpandOverride('component', expandKey)
    ...
    <>
      <WidgetWrapper>
        <div ref={containerRef} className={styles.container} style={sizeStyle}>
          ...
          {resizable && (
            <ResizeHandle targetRef={containerRef} width={width} height={height} onResize={handleResize} />
          )}
        </div>
      </WidgetWrapper>
      {expanded && (
          ...
          initialLayout={[[
            {
              ...
              render: () => useIframe
                ? (
                    <iframe
                      src={iframeSrc}
                      className={styles.immersiveIframe}
                      title={exportName || 'Component widget'}
                      sandbox="allow-same-origin allow-scripts"
                      onLoad={(e) => e.target.blur()}
                    />
