...
A wrapped version of Primer React's `TextInput` that integrates with [`StoryboardForm`](./StoryboardForm.jsx.md). Inside a form, it buffers values in local state and registers with the form's draft system. Override values serve as initial defaults. Outside a form, it behaves as a normal controlled Primer `TextInput`.
...
2. Reads override value via `useOverride(path)` as the initial default
...
The same pattern (local draft + form context + override fallback) is shared by all wrapped input components: `Textarea`, `Select`, and `Checkbox`.
