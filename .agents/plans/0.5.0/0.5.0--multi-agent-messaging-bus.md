...
- **Hub roles** — role-driven behavior for agents in a hub (user-facing name for cluster). The connector-origin agent in a 3+ agent hub is the initial **leader**; other agents are **members**. Roles are loaded from `.agents/roles/*.role.md` metadata (`title`, `type`, `default`) and can be reassigned dynamically.
...
```jsonc
{
  ...
  "clusters": [
    {
      "clusterId": "cluster_abc123",
      "role": "leader",              // role id from .agents/roles (e.g. "leader", "member", "passive")
      "goal": "Get ideas for next week's planning",
      "peers": [
...
1. **Role bootstrapping and role changes (leader/member):**
   - Read role metadata from `.agents/roles/*.role.md` (`title`, `type`, `default`)
   - Follow role-specific instructions from `.agents/roles/leader.role.md` or `.agents/roles/member.role.md`
   - On role changes, re-apply role instructions via tmux send-keys injection

2. **On receiving a user message (priority holder):**
...
3. **On receiving a message token:**
   - Read the message log to see the original message + prior peer responses
   - Formulate response via `POST /messages/respond`

4. **After all message tokens resolve:**
   - Priority holder reads all responses
   - Decides: assign cluster token to a peer, send another message, or signal finality

5. **Leader-owned completion + canvas rendering:**
   - The leader is responsible for deciding when the initial/collective user request is complete.
   - The leader is responsible for rendering the task result on the canvas (create/update the visible output widgets).
   - The leader emits finality only after the canvas reflects the completed outcome; finality may include a summary of what was rendered.
...
### ✅ Phase 0: Bus Core (DONE — `0.5.0--messaging-bus` branch)

The foundational messaging bus is fully implemented and tested:

| Module | Status | Location |
|--------|--------|----------|
| `schema.js` | ✅ Done | `packages/storyboard/src/core/messaging/schema.js` |
| `bus.js` | ✅ Done | `packages/storyboard/src/core/messaging/bus.js` |
| `jsonl-adapter.js` | ✅ Done | `packages/storyboard/src/core/messaging/storage/jsonl-adapter.js` |
| `toon.js` | ✅ Done | `packages/storyboard/src/core/messaging/toon.js` |
| `routes.js` | ✅ Done | `packages/storyboard/src/core/messaging/routes.js` |
| `index.js` | ✅ Done | `packages/storyboard/src/core/messaging/index.js` |

**What's built:** ULID-based envelope schema, singleton pub/sub bus, JSONL persistence (one file per channel), TOON wire format, HTTP endpoints for publish/read/subscribe (SSE), WebSocket push, 65 passing tests.

**Key design:** Storage uses per-channel JSONL files at `.storyboard/messages/channels/`, NOT per-canvas. Channel names use `:` scoping (e.g., `terminal:main:canvas-1:widget-a`) and are sanitized to `--` for filenames. This is a simpler model than the original plan's per-canvas single-file approach — it avoids contention on a single file and makes per-channel reads O(1) instead of requiring log replay + filtering.

### ✅ Phase 0.5: Bus-Backed Terminal Messaging (DONE — `0.5.0--messaging-bus` branch)

The old `pendingMessages` + tmux system has been replaced:

| Component | Status | What it does |
|-----------|--------|-------------|
| `presence.js` | ✅ Done | Heartbeat presence registry (30s beat, 90s expiry, crash-safe) |
| `delivery.js` | ✅ Done | Server-side bridge: bus → tmux send-keys, durable cursors |
| `terminal/send` rewrite | ✅ Done | Server resolves channel, publishes to bus, waits for ack |
| CLI rewrite | ✅ Done | `storyboard terminal send` posts to server endpoint |
| Startup migration | ✅ Done | One-time migration of old pendingMessages to bus |
| `leavePresence()` | ✅ Done | Heartbeat cleanup on ws close/error |
| Tests | ✅ Done | Presence (12 tests), delivery (13 tests), bus (13 tests), schema (16 tests), adapter (11 tests) |

**Current messaging topology:** Each agent has its own inbox channel (`terminal:{branch}:{canvasId}:{widgetId}`). Delivery is always 1-to-1 — Agent A sends to Agent B's specific inbox. However, an agent CAN have multiple peers (the `B--A--C` pattern works): A has both B and C in its `messaging.peers[]` array and can send to either. Each peer relationship is governed by the connector's `messagingMode` (`none`, `one-way`, `two-way`).

**What's NOT built yet:** Group channels, cluster orchestration, message/cluster tokens, conversations, priority holders, finality signals. The bus infrastructure supports arbitrary channel names and could power group channels, but nothing wires that up.

### ✅ Phase 1: Cluster Infrastructure (DONE — `0.5.0--messaging-bus` branch)

Cluster lifecycle and token management are implemented:

| Module | Status | Location |
|--------|--------|----------|
| `cluster-manager.js` | ✅ Done | `packages/storyboard/src/core/messaging/cluster-manager.js` |
| `token-manager.js` | ✅ Done | `packages/storyboard/src/core/messaging/token-manager.js` |

**What's built:** Connected component computation, cluster materialization from connectors, cluster token transfer, goal setting (leader-role-gated), conversation lifecycle (start/finality/reopen), ordered message token assignment/resolution with timeout enforcement, round status tracking.

**Key design:** Leader role (persistent) owns finality and goal-setting. Cluster token (transient speaking rights) is transferable to any member. These are fully decoupled — a member can hold the cluster token for delegated work without gaining finality/goal powers. Token status is injected via tmux separately from role instructions.

### ✅ Phase 2: Server Integration (DONE — `0.5.0--messaging-bus` branch)

| Component | Status | What it does |
|-----------|--------|-------------|
| Cluster routes | ✅ Done | 8 endpoints: hub/send, hub/respond, hub/token, hub/goal, conversation/start, conversation/finality, conversation/reopen, cluster/:canvasId state |
| server.js hooks | ✅ Done | Connected component computation, role materialization, role-change tmux injection, /roles endpoint, connector meta passthrough |
| terminal-config extension | ✅ Done | `role` and `clusters` fields in config lifecycle |
| Config schema | ✅ Done | `canvas.messaging` section (timeout, poll interval, max messages) |

### ✅ Phase 3: Agent Integration (DONE — `0.5.0--messaging-bus` branch)

| Component | Status | What it does |
|-----------|--------|-------------|
| Role files | ✅ Done | `leader.role.md`, `member.role.md`, `cluster-token.md` with full cluster protocol instructions |
| hub-roles.js | ✅ Done | Role loader from `.agents/roles/` with YAML frontmatter parser |
| Role injection | ✅ Done | Role + cluster token status injected via tmux send-keys at startup and on change |
| Client UI | ✅ Done | Role selector dropdown in WidgetChrome, leader crown icon, unique-role enforcement |

### ✅ Phase 4: Polish & Safety (DONE)

12. **Timeout enforcement** — `hub-maintenance.js` runs a background interval checking for conversations idle beyond `conversationTimeoutMinutes` (default 30m). Timed-out conversations are auto-finalized with a `conversation:timeout` bus event. Token timeouts are handled per-token by `token-manager.js`.
13. **HMR push notifications** — Already wired in `server-plugin.js` via `subscribeAll()` → `storyboard:message` HMR event. All bus events are pushed to browser clients in real-time.
14. **Cleanup** — `dissolveHubsForCanvas()` in `hub-manager.js` cleans up all hubs and token state for a canvas. Exposed via `POST /_storyboard/messages/hub/dissolve` route. Maintenance loop stopped on server close via `stopMaintenance()`.

### ✅ Phase 5: Hub Roles & Leader UX (MERGED INTO PHASE 3)

Roles are now treated as part of **Agent Integration**, not a late standalone phase.  
All role work (role metadata loading, leader/member instruction injection, unique-role reassignment, role selector UI, and leader crown UX) is tracked under **Phase 3**.

### ✅ Phase 6: Agent-Spawned Hubs & Broadcast (DONE)

Agents can programmatically create multi-agent hubs: spawn new agents, connect them, assign roles, and manage broadcast state.

#### 6.1 Broadcast Integration

Broadcasting already exists as a canvas server feature (`POST /_storyboard/canvas/broadcast`). It toggles `messagingMode` on connectors and injects tmux notifications to affected terminals.

**Existing endpoint:** `POST /_storyboard/canvas/broadcast`
- `{ name, widgetId, mode, passThrough }` — sets messaging mode on connectors
- `mode: "two-way" | "one-way" | "none"` — controls messaging direction
- `passThrough: true` — BFS across full connected component (not just direct neighbors)
- Injects `📡 [Broadcast ACTIVE/disabled]` via tmux send-keys

**Hub integration:** When a hub is formed or an agent spawns a hub, the leader calls the existing broadcast endpoint with `passThrough: true` to enable messaging across the full hub. No new broadcast endpoints are needed on the messaging routes — the hub manager just needs to track whether broadcast is active for the hub by reading connector `messagingMode` state.

**What's needed:**
- `HubState` gains a `broadcastActive: boolean` field, derived from connector state (are the hub's connectors in `two-way` or `one-way` mode?)
- Terminal config includes `broadcastActive` so agents know whether to poll
- When hub is materialized, broadcast state is computed from connector messaging modes
- New agents joining an active-broadcast hub see `broadcastActive: true` in their config immediately

#### 6.2 Agent Spawn API

Agents call canvas server endpoints to create new agents programmatically:

1. `POST /_storyboard/canvas/:canvasId/widget` — Create a new agent widget (`type: "agent"`, `agentType: "copilot"` etc.). Server creates widget, pre-reserves terminal config, starts tmux session.
2. `POST /_storyboard/canvas/:canvasId/connector` — Connect spawner to new widget. Triggers hub recomputation.
3. `POST /_storyboard/canvas/:canvasId/role` — Assign role to new widget. Server injects role via tmux.
4. Poll readiness signal (existing terminal-server mechanism).
5. `POST /_storyboard/messages/hub/broadcast/open` — Open broadcast to start messaging.

**Guardrails:**
- `canvas.messaging.maxHubSize` config caps the number of members per hub
- Spawned agents are always members unless explicitly promoted to leader
- Leader retains finality ownership — spawning doesn't transfer leadership

#### 6.3 `create-hub` Skill

A terminal-agent skill (`.agents/skills/create-hub/SKILL.md`) that handles hub creation as a single workflow.

**Triggers** (explicit user intent only):
- "Create a hub for…"
- "Spawn a hub with…"
- "Set up a hub to…"
- "Hub: [task description]"

Agents do NOT autonomously decide to create hubs. The user explicitly requests one.

**Workflow:**
1. User asks agent to create a hub with a task description
2. Agent invokes `create-hub` skill
3. Skill determines which roles/agent types are needed based on the task
4. Skill spawns agents, connects them, assigns roles
5. Skill opens broadcast
6. Skill returns hub context to the originating agent (now leader)
7. Leader begins orchestrating via the standard hub messaging protocol

**Skill input:** task description + optional role breakdown hints
**Skill output:** hubId, member list, broadcast status — ready to send messages

#### 6.4 Delegation Chains (Token Forwarding)

Within a flat hub, members can delegate work to downstream peers via token forwarding:

1. Leader sends request → A gets a message token
2. A needs B's help → A creates a **sub-request** via `hub/send` targeting B, with `parentMessageId` referencing the original
3. A's original message token enters `delegating` status (held open, not resolved)
4. B responds to A's sub-request
5. A synthesizes B's response, then resolves its original token

**Schema additions:**
- `parentMessageId` field on message envelopes — traces the delegation tree
- `delegating` token status in token-manager — active but waiting on a sub-request

No sub-clusters needed. The delegation tree emerges from the message correlation chain.

---

## Architecture Delta: Plan vs. Implementation

The original plan assumed a single JSONL per canvas with cluster filtering. The implementation uses **per-channel JSONL files** instead. Here's how this affects the plan:

| Plan concept | Implemented as | Notes |
|-------------|----------------|-------|
| `.storyboard/messages/{canvasId}.jsonl` | `.storyboard/messages/channels/{channel}.jsonl` | Per-channel, not per-canvas |
| Cluster state from single-file replay | Will need per-cluster channel + materializer | Cluster events go to `cluster:{canvasId}:{clusterId}` |
| `GET /messages/:canvasId` | `GET /messages/read/:channel` | Channel-based, not canvas-based |
| Message tokens in envelope | Not yet implemented | Will be added to `message:send` events |
| `canvasId` in every event | `channel` replaces it as the routing key | Events still carry `canvasId` in their body when relevant |

The per-channel approach is simpler for the common case (1-to-1 messaging, presence queries) and doesn't require filtering a large shared log. For clusters, a dedicated cluster channel will aggregate cluster-level events while individual delivery continues through per-widget inbox channels.
...
3. **Role transfer (supersedes cluster priority):** Hubs use explicit roles. The initial leader is derived from connector origin in 3+ agent hubs. Unique-role reassignment enforces single-leader semantics and falls back to the default role when leadership is lost.
