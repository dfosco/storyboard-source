...
### 🔲 Phase 1: Cluster Infrastructure (NOT STARTED)

This is the next major phase. Now that the bus core exists, clusters can be built on top of it:

1. **cluster-manager** — Connected component computation from connectors, cluster lifecycle (create/dissolve/split), priority assignment
2. **token-manager** — Message token assignment/resolution, cluster token transfer, timeout enforcement
3. **Conversation lifecycle** — Start/finality/reopen/timeout

**Key question before starting:** The bus uses per-channel JSONL files. Clusters will need a shared channel (e.g., `cluster:{canvasId}:{clusterId}`) where all cluster events go, alongside the per-widget inbox channels for delivery. This is a design decision to make — do cluster-level events (token assignments, finality signals) go on a shared cluster channel, or do they fan out to individual inbox channels?

### 🔲 Phase 2: Server Integration (NOT STARTED)

5. **message-routes extension** — Add cluster-specific endpoints (send with tokens, respond, finality, etc.)
6. **server.js hooks** — Wire connector add/remove to cluster manager
7. **terminal-config extension** — Add `clusters` array to terminal config
8. **config schema** — Add `canvas.messaging` section to configSchema.js

### 🔲 Phase 3: Agent Integration (NOT STARTED)

9. **Agent skill update** — Update terminal-agent and prompt-agent with cluster protocol instructions
10. **Startup context** — On agent connect, deliver cluster context + any pending message tokens
11. **Role system integration (moved earlier)** — integrate leader/member roles into agent behavior:
   - load role definitions from `.agents/roles/*.role.md` metadata
   - enforce unique roles (e.g., leader) with default fallback
   - inject role-specific instruction dumps into tmux on connect and on role change
   - add leader crown indicator + role selector UX wiring used by agent behavior
12. **Canvas widget rule** — Already done: agents don't create widgets for message sends (exception to Prime Directive)
