...
The existing messaging bus plan (`.agents/plans/0.5.0/0.5.0--multi-agent-messaging-bus.md`) is architecturally sound for the canvas terminal-agent use case but needs two structural changes to meet the user's requirements: **(1) adopt HTTP-like request/response semantics** with correlation IDs, status codes, and explicit request/response pairing (inspired by JSON-RPC 2.0 / MCP's transport pattern, as used by Agentation); and **(2) extract the core bus as a reusable internal service** with a storage abstraction layer and domain adapter pattern, so the same infrastructure can power canvas messaging, annotation passing, widget-to-agent communication, and a true notification system. Agentation's dual-server/shared-store architecture[^1] and the MCP JSON-RPC correlation pattern[^2] provide the design blueprint; Storyboard's existing JSONL materializer[^3], route handler registry[^4], and HMR event channel[^5] provide the implementation foundation.
...
9. [Implementation Impact on Existing Plan](#9-plan-delta)
10. [Confidence Assessment](#10-confidence)
...
| **No storage abstraction** | JSONL is hardcoded — can't swap for in-memory (tests) or SQLite (scale) | Agentation's store abstraction[^12] |
...
packages/storyboard/src/core/messaging/
  ...
  storage/
    ...
    jsonl-adapter.js      — JSONL file backend (default for v1)
    memory-adapter.js     — in-memory backend (tests, hot reload)
  adapters/
    ...
    annotations.js        — widget↔agent annotation passing (v2)
  routes.js               — HTTP route handlers for all adapters
...
graph TB
    subgraph "Bus Core"
        BUS[bus.js<br/>publish / subscribe / read]
        SCHEMA[schema.js<br/>envelope validation + ULID]
        BUS --> SCHEMA
    end

    subgraph "Storage Layer"
        JSONL[jsonl-adapter.js<br/>.storyboard/messages/*.jsonl]
        MEM[memory-adapter.js<br/>Map&lt;channel, event[]&gt;]
        SQL[sqlite-adapter.js<br/>v2 future]
    end
    ...
    subgraph "Domain Adapters"
        CM[canvas-messaging.js<br/>clusters, conversations, tokens]
        NT[notifications.js<br/>readiness signals, system events]
        AN[annotations.js<br/>widget↔agent annotations v2]
    end
    ...
    subgraph "Consumers"
        ROUTES[routes.js<br/>HTTP API endpoints]
        HMR[HMR Push<br/>storyboard:message-received]
        TERM[Terminal Agents<br/>curl + polling]
    end
...
graph LR
    subgraph "Agentation"
        ...
        HS -->|Store| ST[(SQLite)]
        MS[MCP Server] -->|HTTP GET/PATCH| HS
...
| No storage abstraction | `MessageStorageAdapter` interface with JSONL default |
...
|-------|-------------|-------|
| **Phase 0** (new) | **Core bus + storage** — `bus.js`, `schema.js`, `jsonl-adapter.js`, `memory-adapter.js` | Foundation |
| **Phase 1** | Canvas messaging adapter — clusters, tokens, conversations (existing plan Phase 1-2) | Same as before |
| **Phase 2** | Server integration — routes, terminal config, config schema (existing plan Phase 2-3) | Same as before |
| **Phase 3** | Notifications adapter — readiness signals, agent status, task lifecycle | New |
| **Phase 4** | Annotations adapter — widget↔agent annotation passing | New (v2) |
...
- The JSONL adapter is sufficient for v1; SQLite is a v2 optimization
