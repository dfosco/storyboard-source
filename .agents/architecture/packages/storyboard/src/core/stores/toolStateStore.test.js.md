# `packages/storyboard/src/core/stores/toolStateStore.test.js`

<!--

source: packages/storyboard/src/core/stores/toolStateStore.test.js
...
`toolStateStore.test.js` is the executable spec for [`packages/storyboard/src/core/stores/toolStateStore.js`](toolStateStore.js.md). It covers constant exports, init precedence, runtime mutation, dev-only detection, subscriptions, snapshots, and reset behavior.
...
- The suite explicitly tests the production/local-dev split, which is the most subtle rule in the store.
