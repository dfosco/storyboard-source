# `packages/storyboard/src/core/stores/sidePanelStore.ts`

<!--

source: packages/storyboard/src/core/stores/sidePanelStore.ts
...
`sidePanelStore.ts` manages whether the side panel is open and which tab it shows. The file implements a tiny readable/writable store interface in TypeScript so React and other consumers can subscribe without pulling in another state library.
...
The public API is action-oriented: `openPanel()`, `closePanel()`, and `togglePanel()` update state and keep the `sb-sidepanel-open` class on `<html>` synchronized with the current visibility.
...
```ts
const unsubscribe = sidePanelState.subscribe((state) => {
  console.log(state.open, state.activeTab)
})
...
- [`packages/storyboard/src/core/tools/handlers/inspector.js`](../tools/handlers/inspector.js.md)

- [`packages/storyboard/src/core/ui/CoreUIBar.jsx`](../ui/CoreUIBar.jsx.md)

- [`packages/storyboard/src/core/ui/SidePanel.jsx`](../ui/SidePanel.jsx.md)

## Notes

- Closing preserves `activeTab`, so reopening returns to the last panel context.

- DOM synchronization is guarded for SSR by checking `typeof document`.
