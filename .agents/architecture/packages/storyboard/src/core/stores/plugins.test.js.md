# `packages/storyboard/src/core/stores/plugins.test.js`

<!--

source: packages/storyboard/src/core/stores/plugins.test.js
...
`plugins.test.js` defines the contract for [`packages/storyboard/src/core/stores/plugins.js`](plugins.js.md): plugins are enabled by default, explicit config wins, and callers receive defensive copies of the stored config.
...
- [`packages/storyboard/src/core/stores/plugins.js`](plugins.js.md) — imported via `./plugins.js`.
...
- The suite uses re-init instead of a bespoke reset helper, matching the store's tiny API surface.
