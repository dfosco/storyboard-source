# `packages/storyboard/src/core/stores/paletteProviders.test.js`

<!--

source: packages/storyboard/src/core/stores/paletteProviders.test.js
...
`paletteProviders.test.js` uses Vitest module mocks to verify that [`packages/storyboard/src/core/stores/paletteProviders.js`](paletteProviders.js.md) composes providers correctly without depending on the full runtime graph. The suite focuses on item shapes, submenu flattening, categories, and query behavior.
...
- Mocked loader, action, tool-state, and recent-artifact modules keep the tests deterministic while still exercising the public provider API.
