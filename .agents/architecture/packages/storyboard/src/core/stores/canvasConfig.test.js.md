# `packages/storyboard/src/core/stores/canvasConfig.test.js`

<!--

source: packages/storyboard/src/core/stores/canvasConfig.test.js
...
`canvasConfig.test.js` locks down the normalization and precedence rules in [`packages/storyboard/src/core/stores/canvasConfig.js`](canvasConfig.js.md). It ensures empty config, terminal defaults, and agent-specific overrides behave the same way across future refactors.
...
The suite then exercises `isTerminalResizable()` and `getTerminalDimensions()` across base config, per-agent overrides, partial overrides, and custom fallbacks.
...
- `vitest` — external/runtime dependency.

- [`packages/storyboard/src/core/stores/canvasConfig.js`](canvasConfig.js.md) — imported via `./canvasConfig.js`.
...
- Each test resets module state through `_resetCanvasConfig()`, which keeps the singleton safe to reuse across isolated cases.
