...
1. **Wait for your token** — monitor your message token status by polling `GET /_storyboard/messages/read/{channel}?since={lastId}`. When a `hub:message:request` arrives and your token is active, respond.
2. **Respond promptly** — read the full conversation context, do your work, then respond via `POST /_storyboard/messages/hub/respond` with your analysis, implementation, or feedback.
